...
class USSDHandler
{
    ...
    public function __construct($sessionId, $serviceCode, $phoneNumber, $msgType, $msgBody, $networkCode)
    {
    ...
    {
        /*if (!isset($this->sessionId) || !isset($this->serviceCode) || !isset($phoneNumber) || !isset($msgBody) || !isset($networkCode))
            $this->msgBody = "[01] Invalid request!";
        if (empty($this->sessionId) || empty($this->serviceCode) || empty($phoneNumber) || empty($msgBody) || empty($networkCode))
            $this->msgBody = "[02] Invalid request!";*/
    ...
    {
        $this->msgBody = "This service is available for only MTN and VODAFONE users. Please visit https://forms.rmuictonline.com to buy a form on all networks";
        $this->msgType = '2';
    }
